...
class LoginLogout
{
    ...
    private $_SecureHash = '29362603EF127851203DAF82CA65E5FA5BEF12EE60BFC1D08E16C66270A80EF5';
    ...
    public function login($username = null, $password = null)
    {
        ...
        if ( strlen($username) AND strlen($password) ) {
            $black_list = new UserBann();
            $this -> getUserData("`username` = '" . $username . "'");

            if ( is_array($this -> userdata) AND count($this -> userdata) ) {
                $result = $black_list -> check_username( $this -> userdata['email'], $username );

                $this -> setUrlFromBannStatus($result);
            }
            else {
                $this -> error[] = '1';
            }

            if ( !$this -> userdata OR ($this -> userdata["enabled"] != 'yes') ) {
                $result = $black_list -> check_username( $username );
    ...
    private function returnToBann()
    {
        $this -> openUrl('/banned.php?email=' . ( ($this -> userdata['email'] != '') ? $this -> userdata['email'] : 'empty' ) );
        die();
    }

    private function setUrlFromBannStatus($result)
    {
    ...
    private function checkIpBann()
    {
        $nip   = ip2long( $this -> getIP() );
        $query = "SELECT * FROM `bans` WHERE '" . $nip . "' >= first AND '" . $nip . "' <= last";
        $data  = $this -> db -> queryObjectArray($query);

        if ( is_array($data) AND count($data[0]) ) {
            // Mindestens 1 Treffer
            header("HTTP/1.0 403 Forbidden");
            print("<html><body><h1>403 Forbidden</h1>unautorisierte IP-Addresse!</body></html>\n");
            die();
        }
    }
}
