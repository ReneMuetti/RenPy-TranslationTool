...
class Templater
{
    ...
    */
    ...
    protected function __construct()
    {
        ...
        $this -> _fetchLanguage();
        $this -> _loadGuiTranslationForFooter();
    }
    ...
    private function _loadGuiTranslationForFooter()
    {
        $langs = new Languages();
        $liste = $langs -> getXmlLanguagesForGui();
        
        if ( is_array($liste) AND count($liste) ) {
            $links = array();
            $links[] = '<ul class="footer-lang-list">';
            foreach( $liste AS $language ) {
                $this -> loadTemplate('footer_language_item.htm');
                    $this -> setVariable('footer_lang_script_url', $this -> registry -> baseurl . THIS_SCRIPT . '.php');
                    $this -> setVariable('footer_lang_code',       $language['code']);
                    $this -> setVariable('footer_lang_img_base',   $this -> registry -> baseurl . 'skin/images/flags/1x1/');
                    $this -> setVariable('footer_lang_title',      $this -> registry -> user_lang['languages'][$language['code']]);
                $links[] = $this -> renderTemplate();
            }
            $links[] = '</ul>';
            
            $this -> appendToFooter( implode("\n", $links) );
        }
    }
